...
Each volume is only managed by a single metadata magager, but
...
To take advantage of multiple metadadta managers in PanFS,
...
For each metadata manager, we create a volume and assign the volume to a unique GIGA+ server process.
